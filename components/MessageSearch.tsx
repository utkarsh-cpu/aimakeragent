...
import { useOptimizedSearch, useDebounceCallback, useRenderOptimization } from '../utils/debounce';
...
export function MessageSearch({
  ...
  const searchFunction = useCallback((items: Message[], term: string) => {
    // Apply filters first
  ...
  const {
    ...
    debouncedTerm,
    cacheHit
  } = useOptimizedSearch(messages, searchTerm, searchFunction, {
    delay: 250,
    cacheSize: 50,
    minSearchLength: 1,
  });
  ...
  const navigateToResult = (index: number) => {
    if (index >= 0 && index < searchResults.length) {
      ...
      onHighlightMessage(result.messageId, searchTerm);
    }
  };
